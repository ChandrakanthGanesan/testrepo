...
export class DirectIndentService {
  ...
  PoPath(LoactionId: any, indentdate: any) {
    return this.http.get("http://103.21.76.94:6055/Indent/IndentNo?LoactionId=" + LoactionId + '&indentdate=' + indentdate).pipe(
      catchError(this.handleError)
    );
  }
  IndentPONo(IndentNo: any) {
    return this.http.get("http://103.21.76.94:6055/Indent/IndentPONo?IndentNo=" + IndentNo).pipe(
      catchError(this.handleError)
    );
  }
  Department(LoctionId: any, EmpId: any) {
    return this.http.get("http://103.21.76.94:6055/Indent/Department?LoctionId=" + LoctionId + '&EmpId=' + EmpId).pipe(
      catchError(this.handleError)
    );
  }
  CostCenter() {
    return this.http.get("http://103.21.76.94:6055/Indent/CostCenter").pipe(
      catchError(this.handleError)
    );
  }
  CapexNo(LoctionId: any) {
    return this.http.get("http://103.21.76.94:6055/Indent/CapexNo?LoctionId=" + LoctionId)
  }
  EmpApr(LocationId: any, DeptId: any, com_selename: any) {
    return this.http.get("http://103.21.76.94:6055/Indent/ApprovalEmployees?LocationId=" + LocationId + '&DeptId=' + DeptId + '&com_selename=' + com_selename)
  }
  EmpCat(Empid: any, LoctionId: any) {
    return this.http.get("http://103.21.76.94:6055/Indent/EmployeeCategory?Empid=" + Empid + '&LoctionId=' + LoctionId)
  }
  EmpDet(LoctionId: any, EmpId: any, CategoryId: any) {
    return this.http.get("http://103.21.76.94:6055/Indent/EmpDetalis?LoctionId=" + LoctionId + '&EmpId=' + EmpId + '&CategoryId=' + CategoryId)
  }
