...
export class DirectIndentService {
  ...
  PoPath(LoactionId: any, indentdate: any) {
    return this.http.get("http://192.168.203.59:3000/Indent/IndentNo?LoactionId=" + LoactionId + '&indentdate=' + indentdate).pipe(
      catchError(this.handleError)
    );
  }
  IndentPONo(IndentNo: any) {
    return this.http.get("http://192.168.203.59:3000/Indent/IndentPONo?IndentNo=" + IndentNo).pipe(
      catchError(this.handleError)
    );
  }
  Department(LoctionId: any, EmpId: any) {
    return this.http.get("http://192.168.203.59:3000/Indent/Department?LoctionId=" + LoctionId + '&EmpId=' + EmpId).pipe(
      catchError(this.handleError)
    );
  }
  CostCenter() {
    return this.http.get("http://192.168.203.59:3000/Indent/CostCenter").pipe(
      catchError(this.handleError)
    );
  }
  CapexNo(LoctionId: any) {
    return this.http.get("http://192.168.203.59:3000/Indent/CapexNo?LoctionId=" + LoctionId)
  }
  EmpApr(LocationId: any, DeptId: any, com_selename: any) {
    return this.http.get("http://192.168.203.59:3000/Indent/ApprovalEmployees?LocationId=" + LocationId + '&DeptId=' + DeptId + '&com_selename=' + com_selename)
  }
  EmpCat(Empid: any, LoctionId: any) {
    return this.http.get("http://192.168.203.59:3000/Indent/EmployeeCategory?Empid=" + Empid + '&LoctionId=' + LoctionId)
  }
  EmpDet(LoctionId: any, EmpId: any, CategoryId: any) {
    return this.http.get("http://192.168.203.59:3000/Indent/EmpDetalis?LoctionId=" + LoctionId + '&EmpId=' + EmpId + '&CategoryId=' + CategoryId)
  }
    RawMat(Rawmatname: any, Rawmatid: any) {
      return this.http.get("http://192.168.203.59:3000/Indent/RawMaterial?Rawmatname=" + Rawmatname + '&Rawmatid=' + Rawmatid)
    }
  ...
  Save(DirectIndentSave:any){
    return this.http.post('http://192.168.203.59:3000/Indent/Post_DirectIndent',DirectIndentSave)
  }
}
