...
export class StoreIssueComponent implements OnInit {
  ...
  constructor(private router: Router, private loginservice: LoginService, private date: DatePipe, private toastr: ToastrService, private spinnerService: NgxSpinnerService, private formBuilder: FormBuilder, private service: StoreIssueService) { }
  ngOnInit(): void {
    ...
    this.service.Issuenopath(this.LoactionId).subscribe({
      next: (data: any) => {
        ...
        if (IssuenopathData.length > 0) {
          if (IssuenopathData[0].status !== undefined) {
